import { Controller, Post, Get, Body, Param, HttpException, HttpStatus, Sse, MessageEvent } from '@nestjs/common';
import { WhatsAppService } from './whatsapp/whatsapp.service';
import { fromEvent, map, Observable } from 'rxjs';
import { ApiTags, ApiOperation, ApiResponse, ApiBody, ApiParam } from '@nestjs/swagger';
import { CreateConnectionDto, GeneratePairingCodeDto, SendMessageDto } from './dto/apiDto';
...
export class WhatsAppController {
  ...
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async sendMessage(@Body() sendMessageDto: SendMessageDtoType): Promise<{ message: string }> {
    try {
